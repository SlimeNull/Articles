...
通过运算, 获取字符串在控制台上显示的宽度, 例如中文字符宽2, 英文字符宽1

<!--more-->

在网上找了半天, 唯一一个正儿八经的, 就是通过GBK编码的字节数来推测所占宽度, 但我认为这个有点不大妥当, 例如某些特殊字符, 可能就不被GBK编码包含.
所以, 我这里提供了一个可获取字符串显示宽度的可行方法.
Github 仓库地址: [Null.ConsoleEx](https://github.com/SlimeNull/Null.ConsoleEx)

> 关于刚刚所说根据 GBK 编码的字节数来推测宽度  
> int len = Encoding.GetEncoding("GBK").GetByteCount(str);

### 关于使用:

1. 在 nuget 包管理器安装 NullLib.ConsoleEx (推荐)

2. 直接去 Clone 开头所写明的代码仓库即可, 会不定时更新的.或者, 直接复制上面的代码(最长的那一段).

下面的代码大部分是自动生成的哦~![偷懒](https://img-blog.csdnimg.cn/20210204201122821.png) \
